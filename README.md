...
**免責**: 技術的可能性を確認するための試験実装なので、実装していない機能やバグなどある。あと行き当たりばったり実装なのでソースは滅茶苦茶。そもそも言語仕様を忘れた。

**経緯**: 本当は自分用プリプロセッサ `mwg_pp.awk` の一コンポーネント `mwg_pp:ev2(eval_expr)` を置き換える為に試験的に作っていた `mwg_pp:ev3` の major version 2 だった。遅すぎるので実用にならないと判断され放棄された。
...
- 2017-08-10 C++ のコミュニティーで awk における eval が話題になっていたので調子に乗って公開
