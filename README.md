...
GNU awk 上で動く言語処理系の試験実装。本体は ev3v2.awk。ev3v2.awk は本来 master ブランチで生成される (複数のソースを連結)。

技術的可能性を確認するための試験実装なので (ただの言い訳)、実装していない機能やバグなどある。そもそも言語仕様を忘れた。

本当は自分用プリプロセッサ mwg_pp.awk の 1 コンポーネント mwg_pp.eval2 を置き換える為に試験的に作っていた ev3 の major version 2 だった。遅すぎるので実用にならないと判断され放棄された。
