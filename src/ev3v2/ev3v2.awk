...
function ev3obj_initialize(){
  ...
  EV3OBJ_MEMPTR_ENSURE=FALSE;
  EV3OBJ_MEMPTR_CREATE=TRUE;
  EV3OBJ_MEMPTR_IFHAS=2;
  ...
  TYPE_FUNC=ev3obj_type_define("ev3proto_lambda_function",CLASS_STRUCT);
...
#------------------------------------------------------------------------------
# Primitive Operations of Memory Management

# Allocation/Free and Reference Count
...
function ev3obj_assignScal(dst,type,value, _i,_iN,_key){
  ...
  #   先に inc(value) してから destruct(dst) しないと
  #   value の参照が尽きて破壊される可能性があるので注意する。
  if(type==TYPE_REF)
    _ev3obj_inc(value);
...
function ev3obj_alloc(){
  return _ev3obj_create();
}
function ev3obj_new( _obj){
  _obj=_ev3obj_create();
  ev3obj_universe[_obj,UKEY_TYP]=TYPE_OBJ;
  return _obj;
}
function ev3obj_newObj(obj, _obj){
  _obj=_ev3obj_create();
  ev3obj_assignObj(_obj,obj);
  return _obj;
}
function ev3obj_newScal(type,value, _obj){
  _obj=_ev3obj_create();

  # expanded ev3obj_assignScal(_obj,type,value);
  if(type==TYPE_REF)_ev3obj_inc(value);
  ev3obj_universe[_obj,UKEY_TYP]=type;
  ev3obj_universe[_obj]=value;
...
function ev3obj_getMemberPtr(obj,memberName,creates, _type,_cls,_memptr,_mindex,_errorAt){
  ...
  if(_cls==CLASS_BYREF){
    ...
    if(creates==EV3OBJ_MEMPTR_IFHAS){
      return NULL;
    }else if(creates){
    ...
    _errorAt="ev3obj_getMemberPtr(obj = " ev3obj_dump(obj) ", memberName = '" memberName "')";
    _ev3obj_error("unknown object class: obj::typeid = " _type ", obj::typeid::class = " _cls ". @" _errorAt);
  }
}
...
function ev3obj_getMemberValue(obj,memberName, _memptr){
  # @opti2
  _memptr=obj SUBSEP UKEY_MEM SUBSEP memberName;
  if(_memptr in ev3obj_universe)return ev3obj_universe[_memptr];
...
function ev3proto_initialize(){
  ...
  ev3type_Object_initialize(ev3proto_world);
  ev3type_Array_initialize(ev3proto_world);
  ev3type_Function_initialize(ev3proto_world);
}
...
function ev3proto_new(prototype, _ret){
  _ret=ev3obj_new();
  ev3obj_assignScal(_ret SUBSEP UKEY_PROTO,TYPE_REF,prototype);
  return _ret;
}
# function _ev3proto_setPrototype(obj,prototype, _type,_cls){
#   if(obj SUBSEP UKEY_TYP in ev3obj_universe){
#     _type=ev3obj_universe[obj,UKEY_TYP];
#     _cls=ev3obj_type[_type,EV3OBJ_TKEY_CLS];
#     if(_cls==CLASS_BYREF){
#       ev3obj_assignScal(obj SUBSEP UKEY_PROTO,TYPE_REF,prototype);
#       return;
#     }
#   }
#   _ev3_error("ev3proto (setProto): obj = '" obj "' is not a valid Object instance.");
# }
...
function ev3proto_callFunction(obj,fun,args, _dst,_ftype,_fapply,_args2,_ret){
  ...
  if(_ftype==TYPE_NFUNC){
    if((_dst=ev3eval_nativeFunction_call(obj,ev3obj_universe[fun],args)))
      return _dst;
    else
      ev3eval_null();
  }
  ...
  # fun = lambda function
  if(_ftype==TYPE_FUNC){
    return ev3type_Function_internal_apply(fun,obj,args);
  }
...
function _ev3proto_accessProperty_recurse(obj,proto,memberName,arg, _type,_cls,_member,_getter,_args,_proto){
  ...
  if(proto!=ev3type_Object_prototype){
    _proto=ev3type_Object_prototype;
    if(_proto!=NULL)return _ev3proto_accessProperty_recurse(obj,_proto,memberName,arg);
  }
...
function _ev3proto_isPropertyNameValid_recurse(obj,proto,memberName,_proto){
  ...
  # print "dbgdbg201411: ev3type_Object_prototype = " ev3obj_dump(ev3type_Object_prototype);
  return FALSE;
}
...
function ev3scan_initialize(){
  ...
  # EV3_TYPE_ACCESS=ev3obj_type_define("ev3eval_property",CLASS_STRUCT);
  # ev3obj_structType_defineMember(EV3_TYPE_ACCESS,"obj");
  # ev3obj_structType_defineMember(EV3_TYPE_ACCESS,"memberName");
  # ev3obj_structType_defineMember(EV3_TYPE_ACCESS,"getter");
  # ev3obj_structType_defineMember(EV3_TYPE_ACCESS,"setter");
  # ev3obj_structType_defineMember(EV3_TYPE_ACCESS,"rvalue");
  ...
  ev3scan_init_operator("var"       ,EV3_WT_UNA,11.5);
...
function ev3eval_initialize(_proto, _context){
  ...
  ev3obj_setMemberScal(_context,"+rand" ,TYPE_NFUNC,"math::rand" );
  ev3obj_setMemberScal(_context,"+srand",TYPE_NFUNC,"math::srand");
...
function ev3eval_Math_internal_floor(x, _ix){
  if(x>=0)return x;
  _ix=int(1-x);
  return int(x+_ix)-_ix;
}

function ev3type_Math_dispatch(obj,fname,args, _fname,_i,_c,_f,_x,_y){
  _fname=fname
  sub(/^math::/,"",_fname);
  if(_fname ~ /^(a?(sin|cos|tan)h?|log|exp|(sq|cb)rt|int|floor|ceil|round)$/){
    ...
    else if(_fname=="floor"){_x=ev3eval_Math_internal_floor(_x);}
    else if(_fname=="ceil"){_x=-ev3eval_Math_internal_floor(-_x);}
    else if(_fname=="round"){_x=int(_x+(_x<0?-0.5:0.5));}

    return ev3obj_newScal(TYPE_NUM,_x);
    ...
    return ev3obj_newScal(TYPE_NUM,rand());
  }else if(_fname ~ /^srand$/){
    _c=ev3obj_getMemberValue(args,"+length");
    if(_c==0)
      return ev3obj_newScal(TYPE_NUM,srand());
    else{
      _x=ev3eval_tonumber(args SUBSEP UKEY_MEM SUBSEP "+0");
      return ev3obj_newScal(TYPE_NUM,srand(_x));
    }
  }
}

function ev3type_Object_protptype_toString(obj,args){
  return ev3obj_newScal(TYPE_STR,ev3obj_toString(obj));
}
function ev3type_Object_dispatch(obj,fname,args, _value){
  if(fname=="toString"){
    return ev3type_Object_protptype_toString(obj,args);
  }
}
function ev3type_Object_initialize(world){
  ev3type_Object_prototype=ev3obj_placementNew(world,"Object.prototype");
  ev3obj_setMemberScal(ev3type_Object_prototype,"+toString",TYPE_NFUNC,"ev3type_Object_prototype_toString");
}

function ev3type_String_dispatch(obj,fname,args, _value,_a1){
  if(fname ~ /^to(Lower|Upper)$/){
    ...
    return ev3obj_newScal(TYPE_STR,_value);
  }
  ...
  if(fname ~ /^indexOf$/){
    ...
    return ev3obj_newScal(TYPE_NUM,_value);
  }
...
function ev3eval_nativeFunction_call(obj,fname,args, _fname2,_i,_a,_c,_r,_f,_x,_y,_m,_s,_v,_ret){
  ...
  }else if(match(fname,/^ev3type_Array_prototype_(.+)$/,_m)>=1){
    if((_ret=ev3type_Array_dispatch(obj,_m[1],args)))return _ret;
  }else if(match(fname,/^ev3type_Object_prototype_(.+)$/,_m)>=1){
    if((_ret=ev3type_Object_dispatch(obj,_m[1],args)))return _ret;
  }

  _ev3_error("ev3eval (ev3eval_nativeFunction_call)","BUG (invalid native function): specified function '" fname "' does not exist.");
  return ev3eval_null();
}
...
function ev3type_Function_internal_apply(pfun,pthis,pargs, _expr,_closure,_params,_nparams,_scope,_i,_narg,_arg,_ctxSave,_returnValue){
...
function ev3type_Function_prototype_apply(obj,args, _pthis,_pargs){
  _pthis=ev3obj_getMemberValue(args,"+0");
  _pargs=ev3obj_getMemberValue(args,"+1");
  return ev3type_Function_internal_apply(obj,_pthis,_pargs);
}
function ev3type_Function_dispatch(obj,fname,args){
  if(fname=="apply"){
    return ev3type_Function_prototype_apply(obj,args);
  }
}

function ev3type_Function_initialize(world){
  _proto=ev3obj_placementNew(world,"Function.prototype");
  ev3obj_type[TYPE_FUNC,UKEY_PROTO]=_proto;
  ev3obj_setMemberScal(_proto,"!()",TYPE_NFUNC,"ev3type_Function_prototype_apply");
  ev3obj_setMemberScal(_proto,"+apply",TYPE_NFUNC,"ev3type_Function_prototype_apply");
}
...
function ev3eval_expr_unary_operator(ctx,oword,xarg, _arg){
  if(oword=="var"){
    if(ev3obj_getMemberValue(xarg,"xtype")==EV3_WT_NAME){
      ev3obj_setMemberScal(ev3eval_ctx_scope,"+" ev3obj_getMemberValue(xarg,"oword"),TYPE_NULL);
      _ret=ev3eval_expr(ctx,xarg);
      return _ret;
    }else{
      _ev3_error("ev3eval (operator var)","invalid argument of var.");
      return ev3eval_null();
    }
  }
...
function ev3eval_expr_unary_operator_v(ctx,oword,arg, _varg,_vret,_ret){
  ...
  if(oword ~ /^i/){
    # 後置演算子
    if(oword=="i++"||oword="i--"){
      _ret=ev3obj_newObj(_varg=ev3eval_rvalue(arg));
    ...
    if(oword=="!"){
      ...
      return ev3obj_newScal(TYPE_BOOL,!_varg);
    }
...
function ev3eval_expr_functionCall(ctx,x,xtype, _arginfo,_args,_iN,_last,_oword,_this,_callee,_vcallee,_ret,_accessor){
  ...
  if(xtype==EV3_XT_CALL){
    ...
    _ret=ev3obj_newScal(TYPE_REF,_args);
    ev3obj_release(_args);
  }
...
function ev3eval_expr(ctx,x, _xtype,_oword,_xlhs,_xrhs,_xarg,_ret, _arr,_i,_iN,_callee,_this,_scope,_original_ctx,_owner){
  ...
  if(_xtype==EV3_WT_BIN){
    ...
    return ev3obj_newObj(ev3obj_getMemberPtr(x,"value"));
  }else if(_xtype==EV3_XT_VOID){
    return ev3obj_newScal(TYPE_NULL);
  }else if(_xtype==EV3_WT_CLS){
    # 唯の括弧
    _oword=ev3obj_getMemberValue(x,"oword")
    _xarg=ev3obj_getMemberValue(x,"operand");
